...
class RegressionDrive(SupervisedLearningCore):
    ...
    def _fit_base_model(self, y, X, weights, exposure=None, n_trials=None):
        '''Train a new instance of the base_model.'''
        base_model = self.new_base_model()
        base_model.fit(y=y, X=X, weights=weights)

        return base_model


    def _predict_base_model(self, X, exposure=None, n_trials=None, base_model=None):
