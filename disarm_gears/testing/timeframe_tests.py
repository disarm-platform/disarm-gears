...
class TimeframeTests(unittest.TestCase):
    ...
    def test_outputs(self):
        ...
        # By day, length = 1
        tf_0 = Timeframe(start=start_date, length=1, by='day')
        self.assertIsInstance(tf_0.knots_info, pd.DataFrame)
        self.assertEqual(tf_0.knots_info.shape[0], 1)
        self.assertEqual(tf_0.knots_info.shape[1], 2)
        self.assertEqual(tf_0.start, tf_0.end)
        ...
        # By month
        tf_3 = Timeframe(start=start_date, length=3, step=1, by='month')
        self.assertEqual(tf_3.knots_info.knot[0], 0)
        self.assertEqual(tf_3.knots_info.knot[2], 2)
        self.assertEqual(tf_3.knots_info.shape[0], 3)
        self.assertEqual(tf_3.knots_info.init_date[1], pd.to_datetime('2000-07-15'))
        self.assertEqual(tf_3.end, pd.to_datetime('2000-09-14'))
        ...
        tf_5 = Timeframe(start=None, length=5, step=2, by='year', end=end_date)
        self.assertIsInstance(tf_5.knots_info, pd.DataFrame)
        self.assertEqual(tf_5.knots_info.shape[0], 5)
        self.assertEqual(tf_5.knots_info.init_date[3], pd.to_datetime('2006-06-21'))
        self.assertEqual(tf_5.start, pd.to_datetime('2000-06-21'))
        self.assertEqual(tf_5.end, pd.to_datetime(end_date))
